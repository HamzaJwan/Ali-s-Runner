...
const ENCOUNTERS := {
	FATIMA: {
		...
		"dialogue_steps": [
			...
			{
				"role": ROLE_ALI,
				"text": "فاطمة! ابتسامتك تذكّرني إن حتى أصغر خطوة بخير فيها أثر.",
			},
	...
	ZAINAB: {
		...
		"dialogue_steps": [
			{"role": ROLE_HELPER, "text": "علي، الطريق بدأ يصعب — والشجاعة مش غياب الخوف."},
			{"role": ROLE_ALI, "text": "إذن الشجاعة إنك تمشي برغم الصعوبة؟"},
			{"role": ROLE_HELPER, "text": "بالضبط يا خوي… خطوة واحدة برغم الخوف تساوي كثير."},
	...
	JOMANA: {
		...
		"dialogue_steps": [
			{
				"role": ROLE_HELPER,
				"text": "قريب توصل يا خوي… وأنا شايفاك تجاوزت كل شي بخطوة وكلمة.",
			},
			{"role": ROLE_ALI, "text": "الكلمة برضو لها دور يا جمانة؟"},
			{
				"role": ROLE_HELPER,
				"text": "أيه… الكلمة الطيبة تخلي الطريق أخف. خذ مفتاح الطريق وكمل لبابا.",
			},
	...
	FATHER: {
		...
		"dialogue_steps": [
			{
				"role": ROLE_HELPER,
				"text": "أحسنت يا علي… وصلت وكل خطوة طيبة تركت أثرًا في طريقك.",
			},
			{"role": ROLE_ALI, "text": "الأثر ما يكون إلا بالمحاولة والتوكل يا بابا."},
			{"role": ROLE_HELPER, "text": "بالضبط — اعمل اللي عليك، وتوكل على الله، وكلامك وخطواتك تترك أثرًا طيبًا."},
			{
				"role": ROLE_REWARD,
